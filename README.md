...
**Assumption 1**  
$L_f$ is strongly convex and smooth such that $\exists ~m>0,~M>0$, for any $\boldsymbol{w} \in \text{dom} L_f$, we have:
$$
...
**Theorem 1**  
Suppose **Assumption 1** holds, and suppose the density of the causal matrix $D$ is $\delta$ and the model is initialized with $\boldsymbol{w}_0$. Then for every optimization step $k$, we have:
$$
\begin{aligned}
L_f(\boldsymbol{w}^c(k)) - L_f^\star &\le \delta^k \left( L_f(\boldsymbol{w}(k)) - L_f^\star\right) \\
&\le \frac M2 \left[ \delta  \left( 1- \frac m M \right)\right]^k \| \boldsymbol{w}_{0} - \boldsymbol{w}^\star \|^2_2.
\end{aligned}
$$
