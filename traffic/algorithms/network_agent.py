...
from algorithms.agent import Agent, State
...
class NetworkAgent(Agent):
    ...
    def choose(self, count, if_pretrain):
        ...
        q_values = self.q_network.predict(self.convert_state_to_input(self.state), verbose=0)
        # print(q_values)
        if if_pretrain:
    ...
    def _get_next_estimated_reward(self, next_state):
        ...
        if self.para_set.DDQN:
            a_max = np.argmax(self.q_network.predict(
                self.convert_state_to_input(next_state))[0], verbose=0)
            next_estimated_reward = self.q_network_bar.predict(
                self.convert_state_to_input(next_state), verbose=0)[0][a_max]
            return next_estimated_reward
        else:
            next_estimated_reward = np.max(self.q_network_bar.predict(
                self.convert_state_to_input(next_state), verbose=0)[0])
            return next_estimated_reward
