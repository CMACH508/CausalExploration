...
from algorithms.network_agent import NetworkAgent, conv2d_bn, Selector, State
...
class DeeplightAgent(NetworkAgent):
    ...
    def __init__(self,
        ...
        self.q_network = self.build_network(if_test)
    ...
    def build_network(self, if_test):
        ...
        if not if_test:
            ...
            network.compile(optimizer=RMSprop(lr=self.para_set.LEARNING_RATE_EXPLORE),
                        loss="mean_squared_error")
        network.summary()
    ...
    def get_sample(self, memory_slice, dic_state_feature_arrays, Y, gamma, prefix, use_average):
        ...
        if self.if_test:
            if len_memory_slice > self.sample_batch:
                sample_batch = self.sample_batch
            else:
                sample_batch = len_memory_slice
        else:
            sample_batch = len_memory_slice
